...
ld golden_section_search(auto f, ld a, ld b) {
    ...
    ld x = lerp(b, a, PHI_1), fx = f(x), fy; // lerp(a, b, t) = (1 - t) * a + t * b
    rep(60) {
        ld y = lerp(a, b, PHI_1);
        fy = f(y);
        if (fx < fy) {
...
void solve() {
    P<ld> s0, g0, s1, g1;
    in(s0, g0, s1, g1);
    ld T0 = abs(g0 - s0), T1 = abs(g1 - s1);
    if (T0 > T1) {
        swap(T0, T1);
        swap(s0, s1);
        swap(g0, g1);
    }
    auto v0 = g0 - s0, v1 = g1 - s1;
    v0 /= abs(v0);
    v1 /= abs(v1);
    
    
    out(min({
        golden_section_search([&](ld t) {
            return abs(s0 - s1 + (v0 - v1) * t);
        }, 0, T0),
        golden_section_search([&](ld t) {
            return abs(g0 - s1 - v1 * t);
        }, T0, T1),
    }));
}
int main(){
