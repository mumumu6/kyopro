...
int main() {
    ...
    ans.push_back({{0, 0}, {5101010, 5101010}}); // 初期値

    rep(i, n) {
        vector<pair<pll, pll>> tmp;
        for (auto &pp : ans) {
            auto &[l, r] = pp;
            ...
            if (ly <= p[i] && p[i] < ry) {
                // p[i]が範囲内にある場合
                tmp.push_back({{lx, ly + a[i]}, {p[i], p[i] - ly -  + a[i]}});
            } else if (ry <= p[i]) {
                // p[i]が範囲の右側にある場合
                tmp.push_back({{lx, ly + a[i]}, {rx, ry + a[i]}});
            } else {
                // p[i]が範囲の左側にある場合
                tmp.push_back({{lx, ly - b[i] }, {rx , ry - b[i] }});
            }
        }
    }
}