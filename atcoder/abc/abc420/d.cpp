...
int main() {
    ...
    veccc dist(h, vecc(w, vec(2, INF))); // スイッチのオンオフ
    dist[start.ft][start.sd][0] = 0;

    queue<tuple<ll, ll, ll>> que; // i, j, スイッチのオンオフ
    que.push({start.ft, start.sd, 0});// 0 なら o が通れる

    while (!que.empty()) {
        auto [i, j, sw] = que.front();
        que.pop();
        ll d = dist[i][j][sw];
        ...
        rep(k, 4) {
            ...
            if(a[ni][nj] == '?'){
                ...
                if (chmin(dist[ni][nj][nsw], d + 1)) {
                    que.push({ni, nj, nsw});
                }
                continue;
            } 

            if (a[ni][nj] == 'o' && sw == 1) continue; // スイッチオフのときは通れない
            if(a[ni][nj] == 'x' && sw == 0) continue; // スイッチオンのときは通れない

            if (chmin(dist[ni][nj][sw], d + 1)) {
                que.push({ni, nj, sw});
            }
        }
    }
