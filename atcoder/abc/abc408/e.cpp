...
int main() {
    ...
    priority_queue<pll, vector<pll>, greater<pll>> pq; // コスト、 頂点

    pq.push({0, 0}); // {距離, 頂点}

    vecc dist(n, vec(0,INF)); // 各頂点への最短距離を保持
    dist[0].pb( 0); // 始点の距離は0
    ...
    while (!pq.empty()) {
        ...
        sort(all(dist[v])); // dist[v]をソートして最小値を取得しやすくする

        if (dist[v][0] < c) continue; // 既に最短距離が確定している場合はスキップ

        for (auto [to, w] : g[v]) {
            ll newcost = c | w;

            pq.push({newcost, to});
            // debug(to, newcost, dist[to]);
        }
    }
    ...
    cout << dist[n - 1] << el; // 頂点n-1への最短距離を出力
}