...
int main() {
    ...
    auto dfs = [&](auto dfs, ll v, ll bit, set<tuple<ll,ll,ll>> used) -> void {
        if (v == n - 1) chmin(ans, bit);
        for (auto [to, w, idx] : g[v]) {
            if (used.count({v, to, idx})) continue;

            set<tuple<ll,ll,ll>> nused = used;
            nused.insert({v, to, idx});

            dfs(dfs, to, w ^ bit, nused);
            // debug(m, bit, w, to, v);
        }
    };
