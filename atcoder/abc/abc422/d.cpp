...
int main() {
    ...
    rep(i, x + 1) {
        ...
        if (r & (1LL << bit)) {
            for (ll j = 0; j < (1LL << bit); j += 1) { a[j * (1LL << (n - bef))  + cnt] += 1; }
            // debug(cnt,bef,len,bit);
            cnt++;
            bef = bit;
        }
    }
    ...
    ans = max(ans, ll(std::ranges::max(aa) - std::ranges::min(aa)));

    while (aa.size() > 1) {
        vec t(aa.size() / 2);

        rep(i, aa.size() / 2) { t[i] = aa[2*i] + aa[2*i + 1]; }

        ans = max(ans, ll(std::ranges::max(t) - std::ranges::min(t)));
