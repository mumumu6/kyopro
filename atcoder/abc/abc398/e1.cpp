...
int main() {
    ...
    auto dfs = [&](auto dfs, ll v, ll p, bool isodd) -> void {
        for (auto u : g[v]) {
            if (u == p) continue; // 親ノードはスキップ

            odd[u] = !isodd;        // 子ノードの奇偶を反転
            dfs(dfs, u, v, !isodd); // 再帰的に子ノードを訪問
        }
    };

    dfs(dfs, 0, -1, false); // 0番目のノードからDFSを開始

    rep(i, n) rep(j, i) {
        if (odd[i] != odd[j] && g2[i][j] != 1) {
            edges.insert({min(i, j), max(i, j)}); // 奇偶が異なる辺を追加
        }
    }
