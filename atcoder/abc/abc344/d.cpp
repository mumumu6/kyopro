...
int main() {
    ...
    ll n;
    cin >> t >> n;
    vector<ll> cnt(t.size() + 1, 4e18);
    cnt[0] = 0; // 初期状態は空文字列で、操作回数は0

    rep(ni, n) {
        ll a;
        cin >> a;
        vector<ll> tmp = cnt; // 現在の操作回数を保持するための一時的な配列
        rep(_, a) {
            string s;
            cin >> s;

            rep(i, t.size() + 1) {
                if (s.size() + i <= t.size() && t.substr(0, i) + s == t.substr(0, i + s.size())) {
                    chmin(tmp[i + s.size()], cnt[i] + 1);
                }
                // string ts = t.substr(0,i) + s;
                // string ts2 = t.substr(0, i + s.size());

                // debug(i, s, ts, ts2, cnt[i], cnt[i + s.size()]);
            }
        }
        swap(cnt, tmp);
    }
