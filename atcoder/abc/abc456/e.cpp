...
int main() {
    ...
    rep(ti, t) {
        ...
        vector<vector<bool>> visit(w, vector<bool>(n, false));
        rep(wi, w) {
            rep(i, m) {
                auto [u, v] = edge[i];

                if (s[u][wi] == 'o' && s[v][(wi + 1) % w] == 'o') { g[wi][u].push_back(v); }
                if (s[v][wi] == 'o' && s[u][(wi + 1) % w] == 'o') { g[wi][v].push_back(u); }
            }

            rep(i, n) {
                if (s[i][wi] == 'o' && s[i][(wi + 1) % w] == 'o') { g[wi][i].push_back(i); }
            }
        }
        ...
        while (!que.empty()) {
            ...
            if (temp != cnt) {
                for (auto x : nextset) { ok.insert({x, temp % w}); }
                nextset.clear();
                temp = cnt;
            }
            que.pop();

            if (visit[cnt % w][now]) { continue; };
            nextset.push_back(now);
            if (ok.count({now, cnt % w})) {
                Yes;
                flag = true;
                break;
            }

            visit[cnt % w][now] = true;

            for (auto next : g[(cnt) % w][now]) { que.push({next, (cnt + 1) % w}); }
            if (flag) break;
        }
