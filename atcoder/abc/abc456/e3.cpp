...
int main() {
    ...
    rep(ti, t) {
        ...
        rep(wi, w) {
            ...
            for (auto [u, v] : edge) {
                if (s[u][wi] == 'o' && s[v][nwi] == 'o') { g[wi][u].push_back(v); }
                if (s[v][wi] == 'o' && s[u][nwi] == 'o') { g[wi][v].push_back(u); }
            }

            // 同じ都市に留まる辺
            rep(i, n) {
                if (s[i][wi] == 'o' && s[i][nwi] == 'o') { g[wi][i].push_back(i); }
            }
        }

        // DFS color による有向グラフのサイクル検出
        // color[wi][x]
        // 0: 未訪問
        // 1: 探索中
        // 2: 探索完了
        vector<vector<int>> color(w, vector<int>(n, 0));

        auto dfs = [&](auto dfs, ll x, ll wi) -> bool {
            color[wi][x] = 1; // 今のDFS経路上にいる
            ...
            for (ll y : g[wi][x]) {
                ...
                if (color[nwi][y] == 0) {
                    if (dfs(dfs, y, nwi)) { return true; }
                }
            }

            color[wi][x] = 2; // この頂点から先は調べ終わった
            return false;
        };
