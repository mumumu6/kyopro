...
// -----------------------------------------------------------------------------
//  Helper – squared Euclidean distance in RGB space
// -----------------------------------------------------------------------------
inline double sq(double x) { return x * x; }
inline double dist2(const Paint &a, const Paint &b) {
    return sq(a.red - b.red) + sq(a.green - b.green) + sq(a.blue - b.blue);
}

// -----------------------------------------------------------------------------
//  Pick basis colors : 3 axis‑like + (FPS) until want colors
// -----------------------------------------------------------------------------
vector<int> pick_basis_colors(const vector<Paint> &col, int want = 5) {
...
int main() {
    ...
    const vector<int> basis = pick_basis_colors(own_color, min<ll>(12, k));
    ...
    rep(hi, h) {
        ...
        for (int idx1 = 0; idx1 < B; ++idx1)
            for (int idx2 = idx1 + 1; idx2 < B; ++idx2)
                for (int idx3 = idx2 + 1; idx3 < B; ++idx3) {
                    const Paint &color1 = own_color[basis[idx1]];
                    const Paint &color2 = own_color[basis[idx2]];
                    const Paint &color3 = own_color[basis[idx3]];
                    ...
                    rep(dis, 4) {
                        ...
                        rep(ca, 7) rep(cb, 7) rep(cc, 7) {
                            ...
                            double n_error =
                                sqrt(sq(r - tr.red) + sq(g - tr.green) + sq(b - tr.blue)) * 10000 +
                                (ca + cb + cc + now_color.count - use_now_color) * d;

                            if (n_error < best_error) {
                                best_error = n_error;
                                usecount_a = ca;
                                usecount_b = cb;
                                usecount_c = cc;
        ...
        if(usecount_a > 3 || usecount_b > 3 || usecount_c > 3) {
            cout << "Error: Too many colors used!" << el;
        }
