...
vector<int> build_route_with_bias(const vector<vector<int>> &g, const vector<pll> &points, int n, ll T) {
    ...
    for (int i = 0; i + 1 < (int)points.size(); ++i) {
        int s = idx(points[i].first, points[i].second);
        int t = idx(points[i + 1].first, points[i + 1].second);
        auto path = weighted_shortest_path(g, s, t, usage, n);
...
int main() {
    ...
    vector<int> route = build_route_with_bias(g, points, n, T);
    ...
    for (int i = 0; i < steps; ++i) step_dir[i] = dir_from_cells(route[i], route[i + 1], n);
    ...
    for (int i = 0; i < (int)route.size(); ++i)
        if (visit_cnt[route[i]] > 1) is_hub[i] = true;
    ...
    auto evaluate = [&](const vector<int> &sel) -> optional<pair<Solution, int>> {
        Grouping grouping = assign_groups(step_dir, segment_of_step, segment_pos, segment_len, sel);
