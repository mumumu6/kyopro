...
int main() {
    ...
    // その機械の下で今なんこ生産してるかをもつ
    map<ll, pll> cnt; // key: level*n+machine , value: {その下の生産数, -コスト,個数}
    auto index               = [&](int level, int machine) { return level * n + machine; };
    rep(i, n) cnt[i]         = {c[0][i], -1};     // レベル0はひとつ増やすと一個増えるので1扱い
    reps(i, n, l * n) cnt[i] = {c[i / n][i], 0}; // それ以外は0
    ...
    auto update = [&](int id) {
        ...
        cnt[id].second++;
        p[id]++;
        apple -= p[id] * c[level][machine];

        ll cnt[id].first += c[level - 1][machine];
    };
}