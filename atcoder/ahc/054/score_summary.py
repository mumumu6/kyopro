...
DEFAULT_TOOLS_DIR = Path(__file__).resolve().parent / "YDAxDRZr_v2_windows" / "tools_x86_64-pc-windows-gnu"
DEFAULT_TESTER = DEFAULT_TOOLS_DIR / "tester.exe"
...
def collect_inputs(input_dir: Path, limit: int, shuffle: bool) -> list[Path]:
    ...
    if shuffle:
        # Local import to avoid importing random when not needed.
        import random
...
def run_case(tester_cmd: list[str], solver_cmd: list[str], inp: Path, output_dir: Path | None, timeout: float | None, verbose: bool) -> tuple[float | None, str]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Command to launch the tester (e.g. 'cargo run -r --bin tester --')",
    )
    parser.add_argument(
        "--solver",
        default="./a.out",
