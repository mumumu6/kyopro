...
int main() {
    ...
    auto build_plan = [&](const vector<int> &order) -> pair<ld, vector<pll>> {
        ...
        for (int seed : order) {
            for (auto [_, u] : dist[seed]) {
                ...
                cost += llround(sqrt((ld)best_d));
                uf.merge(best_v, u);
            }
            if (uf.size(uf.leader(seed)) < k) return make_pair(INF_LD, vector<pll>{});
        }

        if ((int)edges.size() != n - m) return make_pair(INF_LD, vector<pll>{});
        return make_pair(cost, edges);
    };

    // 多回 max-min で seed を探し、締切までスワップ改良
    const auto margin = chrono::milliseconds(5);
    ld best_cost      = INF_LD;
    vector<pll> bestans;
    vector<int> best_seeds;

    while (chrono::steady_clock::now() + margin < deadline) {
        int first       = rng() % n;
        auto seeds      = build_seeds(first);
        auto [cost, es] = build_plan(seeds);
        if (cost < best_cost) {
            best_cost  = cost;
            bestans    = std::move(es);
            best_seeds = seeds;
        ...
        // 同じ seed 集合で順序を変えて試す（以前の動きの復活）
        shuffle(seeds.begin(), seeds.end(), rng);
        tie(cost, es) = build_plan(seeds);
        if (cost < best_cost) {
            best_cost  = cost;
            bestans    = std::move(es);
    ...
    if (best_seeds.empty()) {
        best_seeds = build_seeds(0);
        tie(best_cost, bestans) = build_plan(best_seeds);
    }

    vector<char> is_seed(n, false);
    for (int s : best_seeds) is_seed[s] = true;

    while (chrono::steady_clock::now() + margin < deadline) {
        int a = best_seeds[rng() % m]; // 置き換え対象の seed

        // 現在の seeds からの最小距離が最大になる非 seed を探して置き換える
        int b       = -1;
        ll best_d2  = -1;
        rep(i, n) {
            if (is_seed[i]) continue;
            ll mind = INF;
            for (int s : best_seeds) {
                if (s == a) continue; // a を抜いた状態で評価
                chmin(mind, dist2[i][s]);
            }
            if (mind > best_d2) {
                best_d2 = mind;
                b       = i;
        ...
        auto [cost, es] = build_plan(cand);
        if (cost < best_cost) {
            is_seed[a] = false;
            is_seed[b] = true;
            best_cost  = cost;
            bestans    = std::move(es);
            best_seeds = std::move(cand);
        }
    }
