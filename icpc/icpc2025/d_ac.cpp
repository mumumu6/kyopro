...
#include <iostream> // 入出力 (std::cin, std::cout, std::nl)
...
// #include <bits/stdc++.h> // 全ての標準ライブラリをまとめてインクルードする場合
// (上記をまとめて代用可能だが、必要なものだけが良い習慣)
...
#define debug_pair(p) cerr << #p << ": (" << (p).first << ", " << (p).second << ")" << nl
...
int main() {
    ...
    while (true) {
        ...
        ll row_cnt = max({first_cnt, cnt, r});
        ll row_r   = r;
        ...
        ll col_cnt = max({first_cnt, cnt, r});
        ll col_r   = r;
