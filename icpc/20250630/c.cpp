...
int main() {
    ...
    rep(i, n) {
        if (h[hmax_id.ft][hmax_id.ft] < h[i][0]) {
            hmax_id = {i, 0};
            chmin(ans, h[hmax_id.ft][hmax_id.sd] - h[hmin_id.ft][hmin_id.sd]);
        };
        if (h[hmin_id.ft][hmin_id.sd] > h[i][0]) {
            hmin_id = {i, 0};
            chmin(ans, h[hmax_id.ft][hmax_id.sd] - h[hmin_id.ft][hmin_id.sd]);
        };
    }
    ...
    while (true) {
        if (hmax_id.sd == n) break;

        ll idx = upper_bound(all(x), h[hmax_id.ft][hmax_id.sd]) - x.begin();
        if (idx == n * n) break;

        hmax_id = {x_id[idx].ft, x_id[idx].sd};
