    ...
    while (true) {
        ...
        rep(i, s.size()) {
            ...
            if (s[i] >= '0' && s[i] <= '9') {
                ...
                if (right) {
                    if (g[g.size() - 2].s == '+') {
                        g[g.size() - 2].mx = g[g.size() - 3].mx + g[g.size() - 1].mx;
                        g[g.size() - 1].mn = g[g.size() - 3].mn + g[g.size() - 1].mn;
                    } else {
                        vector<ll> tmp;
                        tmp.push_back(g[g.size() - 3].mx - g[g.size() - 1].mn);
                        tmp.push_back(g[g.size() - 3].mn - g[g.size() - 1].mx);
                        tmp.push_back(g[g.size() - 1].mx - g[g.size() - 3].mn);
                        tmp.push_back(g[g.size() - 1].mn - g[g.size() - 3].mx);

                        ll mn = tmp[0], mx = tmp[0];
                        rep(j, 4) {
                            if (tmp[j] < mn) mn = tmp[j];
